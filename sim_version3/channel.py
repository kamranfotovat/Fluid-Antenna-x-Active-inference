...
# ----------------------------------------------------------------------------- min-distance feasibility
_DIST_TOL = 1e-9  # float slack so an exactly-d_min grid spacing counts as feasible


def feasible_ports(positions: np.ndarray, selected, remaining, d_min: float | None):
    """Ports in `remaining` that sit at least d_min (wavelengths) from EVERY already-
    selected port -- the hardware min-spacing constraint (mutual coupling / decorrelation).

    positions : (N, 2) coordinates in wavelengths.
    selected  : iterable of already-picked port indices (may be empty).
    remaining : iterable of candidate port indices.
    d_min     : minimum centre-to-centre spacing; None disables the constraint.

    Returns a list. With d_min=None (or no ports selected yet) it is just list(remaining).
    "At least d_min" (>=) is used so an aperture whose grid lands exactly on d_min (e.g.
    lambda/10 grid, d_min = lambda/2 = 5 steps) keeps those ports.
    """
    ...
    if not sel:
        return remaining
    sel_pos = positions[sel]                                   # (|sel|, 2)
    out = []
    for n in remaining:
        d = np.sqrt(np.sum((positions[n] - sel_pos) ** 2, axis=1))   # to each selected
        if np.all(d >= d_min - _DIST_TOL):
            out.append(n)
    return out
