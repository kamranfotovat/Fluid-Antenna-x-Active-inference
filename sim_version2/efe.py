...
def greedy_select(bel, M, S_prev=None, alpha=1.0, beta=1.0, eta_sw=1.0, e_sw=1.0,
    ...
    Min-spacing constraint (positions + d_min): if both are given, at each step the
    candidate pool is HARD-filtered to ports at least d_min from every already-picked port
    (mutual-coupling / decorrelation constraint). This makes the feasible sets an
    independence system rather than a matroid, so the clean (1-1/e) bound no longer strictly
    holds -- we frame it as a greedy heuristic with min-spacing feasibility. If the filter
    ever empties before M ports are chosen (should not happen when M ports fit at d_min in
    the aperture), we fall back to the unconstrained pool for that pick so |S|=M is preserved.
    """
    N = bel.N
    ...
    for _ in range(M):
        ...
        if not pool:                                 # min-spacing exhausted the pool -> relax this pick
            pool = list(remaining)
