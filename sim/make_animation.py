...
def simulate():
    ...
    sim = ChannelSimulator(Nx=Nx, Ny=Ny, K=K, rho=RHO, beta=1.0, seed=SEED)
    ...
    genie = run_genie(H, M, sigma2=SIGMA2, P=1.0)["rate"]
    ...
    def update(t):
        ...
        for n in S:
            ...
            ax_grid.add_patch(Rectangle((x - 0.5, y - 0.5), 1, 1, fill=False,
                                        edgecolor="#39FF14", lw=2.6))
        ax_grid.set_xticks(range(Nx)); ax_grid.set_yticks(range(Ny))
